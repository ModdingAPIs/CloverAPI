...
public static class CharmManager
{
    ...
    public static int MaxId => CustomCharms.Count == 0 ? (int)Identifier.count - 1 : (int)Identifier.count + CustomCharms.Count; // count is included because we add new charms after it
    public static int TotalCharms => (int)Identifier.count - 1 + CustomCharms.Count; // count is included because we add new charms after it
    ...
    public static CharmBuilder Builder(string nameSpace, string identifier) => CharmBuilder.Create(nameSpace, identifier);
	...
	public static PowerupScript SpawnCustomCharm(Identifier identifier, bool isNewGame)
	{
		...
		{
			charmObj = UnityEngine.Object.Instantiate(GetPrefab(charm.GetSourceModel()));
		}
		else
		{
			if (charm.GetModelMode() != CharmBuilder.ModelMode.CustomModel)
			{
				throw new InvalidOperationException($"Unknown model mode for charm {charm.GetName()} (ID: {identifier}).");
			}
			charmObj = AssetBundleUtils.InstantiateCustomModelCharm(charm.GetCustomModelAssetBundlePath(), charm.GetCustomModelName());
		}
		if (charmObj == null)
		{
	...
	internal static Dictionary<Identifier, Identifier> GenerateRemaps(Dictionary<string, Identifier> oldMap, Dictionary<string, Identifier> newMap)
	{
		Dictionary<Identifier, Identifier> remaps = new();
		foreach (KeyValuePair<string, Identifier> pair in oldMap)
		{
			if (newMap.TryGetValue(pair.Key, out var newId))
			{
				if (newId != pair.Value)
				{
