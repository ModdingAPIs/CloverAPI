...
internal class CharmMappings : JsonPersistentData
{
    ...
    [JsonProperty] internal Dictionary<string, PowerupScript.Identifier> Mappings = new();

    public override void BeforeSave()
    {
        this.Mappings = CharmManager.GUIDToId;
    }

    public override void AfterLoad()
    {
        Dictionary<PowerupScript.Identifier, PowerupScript.Identifier> remap =
            CharmManager.GenerateRemaps(this.Mappings);
        if (remap.Count == 0)
        {
        ...
        for (int i = 0; i < GameplayData.Instance.equippedPowerups.Length; i++)
        {
            ...
            PowerupScript.Identifier
                id = PlatformDataMaster.EnumEntryFromString(p1, PowerupScript.Identifier.undefined);
            if (remap.TryGetValue(id, out PowerupScript.Identifier newId))
            {
        ...
        for (int i = 0; i < GameplayData.Instance.equippedPowerups_Skeleton.Length; i++)
        {
            ...
            PowerupScript.Identifier id2 =
                PlatformDataMaster.EnumEntryFromString(p2, PowerupScript.Identifier.undefined);
            if (remap.TryGetValue(id2, out PowerupScript.Identifier newId2))
            {
        ...
        for (int i = 0; i < GameplayData.Instance.drawerPowerups.Length; i++)
        {
            ...
            PowerupScript.Identifier id3 =
                PlatformDataMaster.EnumEntryFromString(p3, PowerupScript.Identifier.undefined);
            if (remap.TryGetValue(id3, out PowerupScript.Identifier newId3))
            {
