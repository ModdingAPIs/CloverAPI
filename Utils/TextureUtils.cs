...
public static class TextureUtils
{
    public static Texture2D LoadTextureFromFile(string filePath, FilterMode filterMode = FilterMode.Point,
        string onNotFound = "error", string onMultipleFound = "warn")
    {
        ...
        if (!tex.LoadImage(bytes))
        {
            throw new Exception("Failed to load texture from file '" + filePath +
                                "'. The file may not be a valid image.");
        }
    ...
    {
        Texture2D copy = new(original.width, original.height, original.format, original.mipmapCount > 1);
