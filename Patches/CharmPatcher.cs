...
internal class CharmPatcher
{
	...
	[HarmonyTranspiler]
	internal static IEnumerable<CodeInstruction> PowerupScript_InitializeAll_Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (instruction.opcode == OpCodes.Ldc_I4 && instruction.operand is int operand && operand == maxId) // No need to check for 'ldc.i4.s' since maxId > 127
            {
                break;
            }
        }
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (instruction.opcode == OpCodes.Callvirt && instruction.operand is MethodInfo methodInfo && methodInfo == AccessTools.Method(typeof(PowerupScript), "Initialize"))
            {
                ...
                yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(CharmPatcher), nameof(InitCustomCharms))); // Call 'InitCustomCharms'
                done = true;
                break;
            }
        }
	...
	[HarmonyTranspiler]
	internal static IEnumerable<CodeInstruction> GameplayData__EnsurePowerupDataArray_Transpiler(IEnumerable<CodeInstruction> instructions)
	{
        var repl = 0;
        ...
        {
            if (instruction.opcode == OpCodes.Ldc_I4 && instruction.operand is int operand && operand == count) // No need to check for 'ldc.i4.s' since count > 127
            {
        ...
        if (repl > 1)
        {
            LogWarning($"GameplayData._EnsurePowerupDataArray transpiler patch replaced {repl} instances of 'ldc.i4 {count}'. Expected to replace exactly 1 instance, but could be fine if an update added more.");
        }
    }
